...
beforeAll(async () => {
  ...
  secondUserToken = `Bearer ${res2.body.token}`;
});
...
test('Pictures get request returns correct data', async () => {
  ...
  expect(response.body[0].location).toStrictEqual({
    lat: 60.1699,
    lng: 24.9384,
    city: 'Helsinki',
    country: 'Finland',
...
test('Voting without token returns 401', async () => {
  const res = await api
    .post(`/api/pictures/${pictureID}/vote`)
    .send({ dir: 1 })
    .expect(401);
});
...
test('Deleting vote without token returns 401', async () => {
  const res = await api.delete(`/api/pictures/${pictureID}/vote`).expect(401);
});

test('User can delete vote', async () => {
  const res = await api
    .delete(`/api/pictures/${pictureID}/vote`)
    .set('Authorization', token)
    .expect(200);
});
...
test('Trying to delete other users picture returns 401', async () => {
  await api
